...
type XdfDatentyp = 'text' | 'date' | 'datetime' | 'num_int' | 'num_gk' | 'bool' | 'file';
type XdfFeldart  = 'input' | 'select' | 'label';

function mapDatentyp(schema: any): XdfDatentyp {
...
function mapFeldart(schema: any): XdfFeldart {
...
function elem(tag: string, content: string, attrs: Record<string, string> = ''  as any): string {
  const attrStr = typeof attrs === 'object'
...
export function exportToXdf(state: FieldAwareState, opts: XdfExportOptions = {}): string {
  const { schema }  = state;
  const s           = schema as any;
  const gruppenId   = opts.gruppenId ?? 'G000000001';
  const version     = opts.version ?? s['x-version'] ?? '1.0';
  const title       = s.title       ?? 'Unbenanntes Formular';
  const description = s.description ?? '';
  const publisher   = s['x-publisher']   ?? '';
  const legalBasis  = s['x-legal-basis'] ?? '';

  // Datenfelder generieren
  let feldIndex = 1;
  const felder = Object.entries(schema.properties ?? {}).map(([key, raw]) => {
    const fs   = raw as any;
    ...
    if (fs.minLength) einschraenkungen += elem('xdf:minLength', String(fs.minLength));
    if (fs.maxLength) einschraenkungen += elem('xdf:maxLength', String(fs.maxLength));
    if (fs.minimum  !== undefined) einschraenkungen += elem('xdf:minimum', String(fs.minimum));
    if (fs.maximum  !== undefined) einschraenkungen += elem('xdf:maximum', String(fs.maximum));
    if (fs.pattern)  einschraenkungen += elem('xdf:pattern',   esc(fs.pattern));

    let codelisteRef = '';
    if (fs.enum?.length) {
      const werte = (fs.enum as unknown[]).map((v) =>
...
export function downloadXdf(state: FieldAwareState): void {
  ...
  const name = ((state.schema as any).title ?? 'formular').replace(/[^a-z0-9]/gi, '_').toLowerCase();
  a.href     = url;
