...
export function StructuralPropertiesPanel({
  selectedScope, uiSchema, dispatch,
}: StructuralPropertiesPanelProps) {
  const { fieldState } = useEditorContext();
  const { t } = useI18n();

  function findElDeep(elements: any[], key: string): any {
    for (const e of elements) {
      if (e.scope === key || e.id === key) return e;
      if (e.columns) for (const col of e.columns) { const f = findElDeep(col, key); if (f) return f; }
      if (e.children) { const f = findElDeep(e.children, key); if (f) return f; }
    }
  }
  const el = findElDeep(uiSchema.elements as any[], selectedScope);
  if (!el) return null;
  ...
  const isLayout   = el.type === 'HorizontalLayout' || el.type === 'ColumnContainer';

  const updateElement = (patch: Record<string, unknown>) => {
    function patchDeep(elements: any[]): any[] {
      return elements.map((e: any) => {
        if (e.scope === selectedScope || e.id === selectedScope) return { ...e, ...patch };
        if (e.columns) return { ...e, columns: e.columns.map((col: any[]) => patchDeep(col)) };
        if (e.children) return { ...e, children: patchDeep(e.children) };
        return e;
      });
    }
    dispatch(createSetFieldStateAction({
      ...fieldState,
      uiSchema: { ...uiSchema, elements: patchDeep(uiSchema.elements as any[]) },
  ...
  return (
    <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Typography variant="subtitle2" sx={{ color: 'text.secondary', fontWeight: 500 }}>
        {isHeader ? 'Abschnittskopf' : isAnnotation ? 'Annotation' : isGroup ? t.properties.gruppe : isLayout ? t.properties.spaltenLayout : t.properties.textElement}
      </Typography>
      <Divider />
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
            {HEADER_COLORS.map(({ bg, text, label }) => (
              <Box
                ...
                onKeyDown={(e) => e.key === 'Enter' && updateOptions({ bgColor: bg, textColor: text })}
                sx={{
                  width: 32, height: 22, borderRadius: 1, cursor: 'pointer',
                  backgroundColor: bg, border: '2px solid',
                  borderColor: el.options?.bgColor === bg ? 'primary.main' : 'divider',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  '&:hover': { transform: 'scale(1.1)' }, transition: 'transform 0.1s',
                }}
              >
                <Typography sx={{ fontSize: '0.55rem', color: text, fontWeight: 700 }}>Aa</Typography>
              </Box>
            ))}
          </Box>
          <Box sx={{
            ...
            <Typography variant="body2" sx={{ color: (el.options?.textColor as string) ?? '#fff', fontWeight: 700 }}>
      ...
      {isLayout && (
        <Box>
          <TextField
            ...
            onChange={(e) => {
              const parts = e.target.value.split(':').map((v) => parseInt(v.trim(), 10)).filter((n) => !isNaN(n) && n > 0);
              if (parts.length >= 2) {
          ...
          <Box sx={{ mt: 1, display: 'flex', gap: 0.5, height: 20 }}>
            {(el.widths ?? [1,1]).map((w: number, i: number) => (
              <Box key={i} sx={{ flex: w, backgroundColor: 'primary.light', borderRadius: 0.5, opacity: 0.6 }}>
                <Typography sx={{ fontSize: '0.55rem', textAlign: 'center', lineHeight: '20px', color: 'white', fontWeight: 700 }}>{w}</Typography>
              </Box>
            ))}
          </Box>
        </Box>
