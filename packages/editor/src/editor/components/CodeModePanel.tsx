...
function buildDisplayUiSchema(fieldState: FieldAwareState): object {
  function convert(el: any): object | null {
    ...
    if ((el.type === 'GroupContainer') && (el.children || el.elements)) {
      return { type: 'Group', label: el.label ?? '', elements: (el.children ?? el.elements ?? []).map(convert).filter(Boolean) };
    }
    if (el.type === 'Label') return { type: 'Label', text: el.label ?? '' };
    if (el.type === 'Control') { const { id: _id, ...rest } = el; return rest; }
    return null;
  }
  return { type: 'VerticalLayout', elements: fieldState.uiSchema.elements.map(convert).filter(Boolean) };
}

/** Parst bearbeitetes uiSchema und konvertiert zurück in internes Format */
function parseUiSchema(raw: any): FieldAwareState['uiSchema'] {
  const elements = raw?.elements ?? [];
  const converted = elements.map((el: any) => {
    const uiEl = fromLegacy(el);
...
export function CodeModePanel({ fieldState, previewData, onFieldStateChange, onPreviewDataChange }: CodeModePanelProps) {
  ...
  const getValue = useCallback((): string => {
    ...
    if (tab === 'uischema')  return JSON.stringify(buildDisplayUiSchema(fieldState), null, 2);
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {error && <Alert severity="warning" sx={{ borderRadius: 0, py: 0.5 }}>{error}</Alert>}
